...
fn test_suggested_shortcuts() {
    ...
    if cfg!(target_os = "macos") {
        ...
        assert!(suggested_shortcuts.iter().any(|s| s.contains("Ctrl") || s.contains("Alt")));
    }
}
...
async fn test_window_state_management() {
    ...
    let window_visible = settings_service.get_setting("window_visible").await.unwrap();
    assert_eq!(window_visible, Some("true".to_string()));
    
    let window_focused = settings_service.get_setting("window_focused").await.unwrap();
...
async fn test_window_management_settings_integration() {
    ...
    let settings_service2 = SettingsService::new(
        Arc::new(DbService::new(&db_path).unwrap())
    );
