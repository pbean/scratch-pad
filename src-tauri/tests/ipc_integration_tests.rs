...
async fn test_note_ipc_commands_integration() {
    ...
    let created_note = state
        .db
        .create_note("Test note from IPC".to_string())
        .await
        .unwrap();
    ...
    let updated_note = state
        .db
        .update_note_content(created_note.id, "Updated via IPC".to_string())
        .await
        .unwrap();
...
async fn test_search_ipc_commands_integration() {
    ...
    let (paginated_results, total_count) = state
        .search
        .search_notes_paginated("note", 0, 10)
        .await
        .unwrap();
    ...
    let (boolean_results, _total_count, complexity) = state
        .search
        .search_notes_boolean_paginated("test OR unique", 0, 10)
        .await
        .unwrap();
    ...
    // Test empty search
    let empty_results = state
        .search
        .search_notes("nonexistent_keyword_xyz")
        .await
        .unwrap();
    ...
    let validation = state
        .search
        .validate_boolean_search_query("test AND unique")
        .unwrap();
...
async fn test_settings_validation_integration() {
    ...
    state
        .settings
        .set_setting("global_shortcut", valid_shortcut)
        .await
        .unwrap();
    ...
    let result = state
        .settings
        .set_setting("layout_mode", "invalid_mode")
        .await;
...
async fn test_error_handling_integration() {
    ...
    let result = state
        .db
        .update_note_content(99999, "content".to_string())
        .await;
    ...
    let validation = state
        .search
        .validate_boolean_search_query("(unbalanced parentheses")
        .unwrap();
    assert!(!validation.is_valid);
...
async fn test_performance_integration() {
    ...
    for i in 0..20 {
        ...
        let handle =
            tokio::spawn(
                async move { db.create_note(format!("Performance test note {}", i)).await },
            );
        handles.push(handle);
    }
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move { search.search_notes(&format!("note {}", i)).await });
    ...
    let (results, total_count) = state
        .search
        .search_notes_paginated("test", 0, 5)
        .await
        .unwrap();
...
async fn test_export_import_integration() {
    ...
    state
        .settings
        .set_bool_setting("bool_setting", true)
        .await
        .unwrap();
    state
        .settings
        .set_int_setting("int_setting", 123)
        .await
        .unwrap();
