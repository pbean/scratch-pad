...
use scratch_pad_lib::{
    error::AppError,
    validation::{OperationCapability, OperationContext, OperationSource, SecurityValidator},
};
...
mod path_validation_tests {
        ...
        for path in &safe_paths {
            ...
            if let Ok(validated_path) = result {
                ...
                assert!(validated_path
                    .to_string_lossy()
                    .contains(&path.replace('/', std::path::MAIN_SEPARATOR_STR)));
            }
        }
    }
    ...
    fn test_validate_export_path_without_base_directory() {
        let safe_paths = ["document.txt", "notes/my-note.md"];

        for path in &safe_paths {
            let result = SecurityValidator::validate_export_path(path, None);
            assert!(
                result.is_ok(),
                "Safe path should be valid without base: {}",
                path
            );
        }
    }
    ...
    fn test_contains_path_traversal_comprehensive() {
        ...
        for pattern in &safe_patterns {
            assert!(
                !SecurityValidator::contains_path_traversal(pattern),
                "Should not detect traversal in safe pattern: {}",
                pattern
            );
        }
    }
    ...
    fn test_allowed_file_extensions() {
        ...
        for file in &allowed_files {
            ...
            assert!(
                result.is_ok(),
                "Should allow file with allowed extension: {}",
                file
            );
        }
    }
    ...
    fn test_dangerous_file_extensions_blocked() {
        ...
        for file in &dangerous_files {
            ...
            assert!(
                result.is_err(),
                "Should reject dangerous file extension: {}",
                file
            );
        }
    }
    ...
    fn test_files_without_extensions_blocked() {
        ...
        let no_extension_files = ["README", "Makefile", "LICENSE", "config"];
...
mod content_validation_tests {
    ...
    fn test_validate_note_content_with_context() {
        ...
        assert!(
            result.is_ok(),
            "Safe content with proper context should pass"
        );
        ...
        assert!(
            result.is_err(),
            "Malicious content should be rejected even with proper context"
        );
        ...
        assert!(
            result.is_err(),
            "Should reject when missing WriteNotes capability"
        );
    }
...
mod search_validation_tests {
    ...
    fn test_validate_search_query_safe_queries() {
        ...
        for query in &safe_queries {
            ...
            assert!(
                result.is_ok(),
                "Safe search query should be valid: '{}'",
                query
            );
        }
    }
...
mod shortcut_validation_tests {
    ...
    fn test_validate_shortcut_length_limits() {
        ...
        assert!(
            result.is_err(),
            "Shortcut over length limit should be rejected"
        );
...
mod id_and_pagination_tests {
    ...
    fn test_validate_pagination_valid_params() {
        ...
        for (offset, limit) in &valid_params {
            ...
            assert!(
                result.is_ok(),
                "Valid pagination should pass: offset={}, limit={}",
                offset,
                limit
            );
        }
    ...
    fn test_validate_pagination_invalid_params() {
        ...
        for (offset, limit) in &invalid_params {
            ...
            assert!(
                result.is_err(),
                "Invalid pagination should be rejected: offset={}, limit={}",
                offset,
                limit
            );
        }
