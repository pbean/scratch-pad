...
async fn test_settings_table_integration() {
    ...
    settings_service.export_settings_to_file(&export_path.to_string_lossy()).await
        .expect("Failed to export settings");
    
    // Clear settings and import again
    settings_service.delete_setting("test_key").await.expect("Failed to delete setting");
    
    // Import settings - Fixed: use import_settings_from_file with string
    settings_service.import_settings_from_file(&export_path.to_string_lossy()).await
        .expect("Failed to import settings");
    
    // Verify setting was restored
    let restored_value = settings_service.get_setting("test_key").await
...
async fn test_fts5_search_integration() {
    ...
    search_service.db_service.create_note(note1_content.to_string()).await.unwrap();
    search_service.db_service.create_note(note2_content.to_string()).await.unwrap();
    search_service.db_service.create_note(note3_content.to_string()).await.unwrap();
    ...
    let (paginated_results, total_count) = search_service.search_notes_paginated("programming", 0, 1).await.unwrap();
    ...
    let validation = search_service.validate_boolean_search_query("programming AND Rust").unwrap();
...
async fn test_concurrent_operations_integration() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            db_clone.create_note(format!("Concurrent note {}", i)).await
        });
        handles.push(handle);
    }
    ...
    for i in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            search_clone.search_notes(&format!("note {}", i)).await
        });
...
async fn test_database_persistence_integration() {
    ...
    // Create initial data
    {
        // Fixed: pass path directly
        let db_service = DbService::new(&db_path).unwrap();
        let note = db_service.create_note("Persistent test note".to_string()).await.unwrap();
