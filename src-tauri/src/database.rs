...
impl DbService {
    ...
    pub async fn create_note_with_context(
        &self,
        content: String,
        context: &crate::validation::OperationContext,
    ...
    pub async fn create_note(&self, content: String) -> Result<Note, AppError> {
        ...
        #[cfg(test)]
        {
            let test_context = crate::validation::OperationContext::new_test(vec![
                crate::validation::OperationCapability::WriteNotes,
            ]);
            let security_validator = crate::validation::SecurityValidator::new();
...
mod tests {
    ...
    async fn test_search_notes() {
        ...
        db.create_note("Rust programming is fun".to_string())
            .await
            .unwrap();
        db.create_note("JavaScript is also good".to_string())
            .await
            .unwrap();
            ...
            .map(|i| {
                tokio::spawn(async move {
                    ...
                    assert_eq!(
                        all_notes.len(),
                        1,
                        "Thread {} should have exactly 1 note, but found {}",
                        i,
                        all_notes.len()
                    );
