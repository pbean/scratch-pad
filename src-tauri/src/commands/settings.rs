...
pub async fn get_all_settings(
    ...
    // Log security event for settings access
    log_security_event("ALL_SETTINGS_READ", "IPC", true, "All settings accessed");
...
/// - Secure logging
#[tauri::command]
pub async fn delete_setting(key: String, app_state: State<'_, AppState>) -> Result<(), ApiError> {
...
pub async fn save_settings(
    ...
    if let Some(layout) = &settings.window_layout {
        ...
        app_state
            .settings
            .set_setting("window_layout", layout)
            .await?;
    }
    ...
    if let Some(auto_save) = settings.auto_save {
        app_state
            .settings
            .set_setting("auto_save", &auto_save.to_string())
            .await?;
    }
    ...
    if let Some(font_size) = settings.font_size {
        ...
        app_state
            .settings
            .set_setting("font_size", &font_size.to_string())
            .await?;
    }
    ...
    if let Some(line_numbers) = settings.line_numbers {
        app_state
            .settings
            .set_setting("line_numbers", &line_numbers.to_string())
            .await?;
    }
    ...
    if let Some(word_wrap) = settings.word_wrap {
        app_state
            .settings
            .set_setting("word_wrap", &word_wrap.to_string())
            .await?;
    }

    // Log security event for settings modification
    log_security_event("SETTINGS_SAVED", "IPC", true, "Application settings saved");
...
/// - Secure logging
#[tauri::command]
pub async fn load_settings(app_state: State<'_, AppState>) -> Result<AppSettings, ApiError> {
    ...
    let auto_save = app_state
        .settings
        .get_setting("auto_save")
        .await?
        .and_then(|s| s.parse::<bool>().ok());

    let font_size = app_state
        .settings
        .get_setting("font_size")
        .await?
        .and_then(|s| s.parse::<u32>().ok());

    let line_numbers = app_state
        .settings
        .get_setting("line_numbers")
        .await?
        .and_then(|s| s.parse::<bool>().ok());

    let word_wrap = app_state
        .settings
        .get_setting("word_wrap")
        .await?
        .and_then(|s| s.parse::<bool>().ok());
...
pub async fn register_global_shortcut(
    ...
    log_security_event(
        ...
        &format!(
            ...
            sanitize_shortcut_for_logging(&shortcut)
        ),
    );
...
mod tests_disabled {
    ...
    async fn create_test_app_state() -> AppState {
        ...
        AppState {
            ...
            global_shortcut: Arc::new(
                GlobalShortcutService::new_test(settings_service.clone())
                    .expect("Failed to create GlobalShortcutService for test"),
            ),
            window_manager: Arc::new(
                WindowManager::new_test(settings_service)
                    .expect("Failed to create WindowManager for test"),
            ),
            plugin_manager,
            security_validator,
    ...
    async fn test_delete_setting() {
        ...
        let _ = app_state
            .settings
            .set_setting("test_delete", "test_value")
            .await;
    ...
    async fn test_global_shortcut_validation() {
        ...
        let valid_shortcuts = vec!["Ctrl+Shift+N", "Alt+F4", "Meta+Space", "Ctrl+Alt+T"];
