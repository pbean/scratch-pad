...
#[tauri::command]
pub async fn is_shutting_down(app_state: State<'_, AppState>) -> Result<bool, ApiError> {
...
#[tauri::command]
pub async fn initiate_shutdown(app_state: State<'_, AppState>) -> Result<(), ApiError> {
...
mod tests_disabled {
    ...
    async fn test_operation_context_validation() {
        ...
        assert!(app_state
            .security_validator
            .validate_operation_context(&system_context)
            .is_ok());
    ...
    async fn test_lifecycle_security_validation() {
        ...
        let context_without_system_access =
            OperationContext::new_test(vec![OperationCapability::ReadNotes]);
        assert!(app_state
            .security_validator
            .validate_operation_context(&context_without_system_access)
            .is_ok());

        // System access should be properly validated
        let context_with_system_access =
            OperationContext::new_test(vec![OperationCapability::SystemAccess]);
        assert!(app_state
            .security_validator
            .validate_operation_context(&context_with_system_access)
            .is_ok());
    }
}
