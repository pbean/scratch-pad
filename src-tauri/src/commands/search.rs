...
use crate::commands::shared::{
    validate_ipc_operation, validate_search_query_secure, 
    CommandPerformanceTracker, log_security_event
};
...
pub async fn search_notes_paginated(
    ...
    log_security_event(
        ...
        "IPC",
        true,
        &format!("Paginated search (page {}, size {}): '{}'", 
                page, 
                page_size,
                if query.len() > 50 { 
                    format!("{}...", &query[..47])
...
pub async fn search_notes_boolean_paginated(
    ...
    if page_size == 0 || page_size > 100 {
        return Err(ApiError {
            code: "VALIDATION_ERROR".to_string(),
            message: "Page size must be between 1 and 100 for Boolean search operations".to_string(),
        });
    }
    ...
    log_security_event(
        ...
        "IPC",
        true,
        &format!("Boolean search (page {}, size {}): '{}'", 
                page, 
                page_size,
                if query.len() > 50 { 
                    format!("{}...", &query[..47])
...
pub async fn validate_boolean_search_query(
    ...
    log_security_event(
        ...
        "IPC",
        true,
        &format!("Validating Boolean query: '{}'", 
            if query.len() > 50 { 
                format!("{}...", &query[..47])
            } else { 
                query.clone() 
...
mod tests_disabled {
    ...
    async fn create_test_app_state() -> AppState {
        ...
        AppState {
            ...
            global_shortcut: Arc::new(GlobalShortcutService::new_test(settings_service.clone()).unwrap()),
    ...
    async fn test_boolean_search_paginated() {
        ...
        let result = app_state.search.search_notes_boolean_paginated("rust OR python", 0, 10).await;
        assert!(result.is_ok());
