...
use crate::validation::{OperationCapability, OperationContext, SecurityValidator};
...
pub fn validate_setting_secure(key: &str, value: &str) -> Result<(), AppError> {
...
mod tests {
    ...
    fn test_validate_note_content_secure() {
        ...
        assert!(validate_note_content_secure(
            &validator,
    ...
    fn test_validate_search_query_secure() {
        ...
        assert!(
            validate_search_query_secure(&validator, "'; DROP TABLE notes; --", &context).is_err()
        );
    }
