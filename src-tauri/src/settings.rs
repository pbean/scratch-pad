...
pub fn get_default_settings() -> HashMap<String, String> {
    ...
    defaults.insert(
        "shortcuts.toggle_window".to_string(),
    ...
    defaults.insert(
        "performance.animation_enabled".to_string(),
        "true".to_string(),
    );
    defaults.insert(
        "performance.virtual_scrolling".to_string(),
        "true".to_string(),
    );
...
mod tests {
    ...
    async fn test_setting_operations() -> Result<(), anyhow::Error> {
        ...
        // Test has_setting
        assert!(service
            .has_setting("test_key")
            .await
            .context("Failed to check if setting exists")?);
        assert!(!service
            .has_setting("nonexistent_key")
            .await
    ...
    async fn test_typed_settings() -> Result<(), anyhow::Error> {
        ...
        service
            .set_bool_setting("test_bool", true)
            .await
            .context("Failed to set boolean setting")?;
        let bool_value = service
            .get_bool_setting("test_bool")
            .await
        ...
        service
            .set_int_setting("test_int", 42)
            .await
            .context("Failed to set integer setting")?;
        let int_value = service
            .get_int_setting("test_int")
            .await
        ...
        // Test float settings
        service
            .set_float_setting("test_float", 3.14)
            .await
            .context("Failed to set float setting")?;
        let float_value = service
            .get_float_setting("test_float")
            .await
    ...
    async fn test_delete_setting() -> Result<(), anyhow::Error> {
        ...
        service
            ...
            .context("Failed to set setting")?;
        assert!(service
            .has_setting("to_delete")
            .await
            .context("Setting should exist")?);

        // Delete the setting
        service
            .delete_setting("to_delete")
            .await
    ...
    async fn test_get_setting_or_default() -> Result<(), anyhow::Error> {
        ...
        let result = service
            .get_setting_or_default("nonexistent", "default")
            .await
    ...
    async fn test_validation() -> Result<(), anyhow::Error> {
        ...
        // Test valid setting
        assert!(service
            .set_setting("valid_key", "valid_value")
            .await
            .is_ok());
        ...
        assert!(service
            .set_setting("key with spaces", "value")
            .await
            .is_err());
    ...
    async fn test_export_import() -> Result<(), anyhow::Error> {
        ...
        // Set some test settings
        service
            .set_setting("export_test1", "value1")
            .await
            .context("Failed to set export_test1")?;
        service
            .set_setting("export_test2", "value2")
            .await
            .context("Failed to set export_test2")?;

        // Export settings
        service
            .export_settings_to_file(&export_file.to_string_lossy())
            .await
