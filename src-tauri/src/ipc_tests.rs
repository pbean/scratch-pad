...
async fn test_note_search_functionality() -> Result<(), AppError> {
    ...
    // Create test notes
    let _note1 = db_service
        .create_note("Rust programming tutorial".to_string())
        .await?;
    let _note2 = db_service
        .create_note("JavaScript development guide".to_string())
        .await?;
    let _note3 = db_service
        .create_note("Python data science".to_string())
        .await?;
...
async fn test_paginated_search() -> Result<(), AppError> {
    ...
    for i in 1..=5 {
        db_service
            .create_note(format!("Search test note {}", i))
            .await?;
    }
    ...
    let (no_results, total_none) = db_service
        .search_notes_paginated("NonExistent", 0, 10)
        .await?;
...
async fn test_note_persistence() -> Result<(), AppError> {
    ...
    let simple_note = db_service
        .create_note("Simple text note".to_string())
        .await?;
    let markdown_note = db_service
        .create_note("# Markdown Header\n\nSome **bold** text".to_string())
        .await?;
    let unicode_note = db_service
        .create_note("Unicode: 🚀 Café naïve résumé".to_string())
        .await?;
    let long_note = db_service
        .create_note("Long content: ".to_owned() + &"x".repeat(1000))
        .await?;
...
async fn test_concurrent_operations() -> Result<(), AppError> {
    ...
    let handles: Vec<_> = (0..10)
        .map(|i| {
            ...
            tokio::spawn(async move { db.create_note(format!("Concurrent note {}", i)).await })
        })
        .collect();
    ...
    let search_handles: Vec<_> = (0..5)
        .map(|_| {
            let db = Arc::clone(&db_service);
            tokio::spawn(async move { db.search_notes("Concurrent").await })
        })
        .collect();
...
async fn test_error_handling() -> Result<(), AppError> {
    ...
    let test_note = db_service
        .create_note("Error recovery test".to_string())
        .await?;
...
async fn test_database_health_check() -> Result<(), AppError> {
    ...
    let _note = db_service
        .create_note("Health check test".to_string())
        .await?;
...
async fn test_note_timestamps() -> Result<(), AppError> {
    ...
    let updated_note = db_service
        .update_note_content(note.id, "Updated timestamp test".to_string())
        .await?;
...
async fn test_note_format_handling() -> Result<(), AppError> {
    ...
    let plain_note = db_service
        .create_note("Plain text content".to_string())
        .await?;
    assert_eq!(plain_note.format, NoteFormat::PlainText);

    let markdown_content = db_service
        .create_note("# Markdown Content\n\nWith **formatting**".to_string())
        .await?;
