...
impl PluginManifest {
    ...
    pub fn new(name: String, version: String) -> Self {
        Self {
            ...
            description: None,
            author: None,
            required_capabilities: vec![OperationCapability::ReadNotes], // Conservative default
...
impl HelloWorldPlugin {
    pub fn new() -> Self {
        let mut manifest =
            PluginManifest::new("Hello World Plugin".to_string(), "1.0.0".to_string());
        manifest.description =
            Some("A simple example plugin that demonstrates the plugin architecture".to_string());
...
mod tests {
    ...
    fn test_plugin_manifest_creation() {
        let manifest = PluginManifest::new("Test Plugin".to_string(), "1.0.0".to_string());
        ...
        assert!(manifest
            ...
            .contains(&OperationCapability::ReadNotes));
    }

    #[test]
    fn test_plugin_capability_validation() {
        let mut manifest = PluginManifest::new("Test Plugin".to_string(), "1.0.0".to_string());

        // Allow WriteNotes capability
        manifest
            .required_capabilities
            .push(OperationCapability::WriteNotes);

        // Should allow ReadNotes (default capability)
        assert!(manifest
            .validate_operation(&OperationCapability::ReadNotes)
            .is_ok());

        // Should allow WriteNotes (explicitly added)
        assert!(manifest
            .validate_operation(&OperationCapability::WriteNotes)
            .is_ok());

        // Should reject DeleteNotes (not in manifest)
        assert!(manifest
            .validate_operation(&OperationCapability::DeleteNotes)
            .is_err());
    }
    ...
    fn test_plugin_frequency_validation() {
        ...
        for i in 0..30 {
            let result =
                manager.validate_plugin_operation(&plugin_name, OperationCapability::ReadNotes);
    ...
    fn test_plugin_not_found_error() {
        ...
        let result =
            manager.validate_plugin_operation("NonExistentPlugin", OperationCapability::ReadNotes);
