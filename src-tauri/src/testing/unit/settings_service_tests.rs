...
use crate::settings::{get_default_settings, SettingsService};
use std::sync::Arc;
use tokio;
...
mod settings_service_integration_tests {
    ...
    async fn test_settings_service_typed_operations() -> Result<(), AppError> {
        ...
        settings_service
            .set_float_setting("float_key", 3.14)
            .await?;
        ...
        settings_service
            .set_setting("string_key", "test_string")
            .await?;
    ...
    async fn test_settings_service_defaults() -> Result<(), AppError> {
        ...
        let bool_default = settings_service
            .get_bool_setting("non_existent_bool")
            .await?;
        ...
        let float_default = settings_service
            .get_float_setting("non_existent_float")
            .await?;
        ...
        let string_with_default = settings_service
            .get_setting_or_default("non_existent_string", "default_value")
            .await?;
    ...
    async fn test_settings_service_json_export_import() -> Result<(), AppError> {
        ...
        settings_service
            ...
            .await?;
        settings_service
            .set_bool_setting("export_bool", true)
            .await?;
    ...
    async fn test_settings_service_error_handling() -> Result<(), AppError> {
        ...
        let result = settings_service
            .import_settings("invalid json content".to_string())
            .await;
        assert!(result.is_err());
    ...
    async fn test_settings_service_concurrency() -> Result<(), AppError> {
        ...
        let handles: Vec<_> = (0..10)
            .map(|i| {
                ...
                tokio::spawn(async move {
                    service
                        .set_setting(&format!("concurrent_key_{}", i), &format!("value_{}", i))
                        .await
                })
            })
            .collect();
        ...
        for i in 0..10 {
            let value = settings_service
                .get_setting(&format!("concurrent_key_{}", i))
                .await?;
    ...
    async fn test_settings_service_security_validation() -> Result<(), AppError> {
        ...
        let result = settings_service
            .set_setting("test_key", "'; DROP TABLE settings; --")
            .await;
        ...
        let result = settings_service
            .set_setting("null_test", "value\0with\0nulls")
            .await;
        // Should be caught by security validation
        assert!(result.is_err());
    ...
    async fn test_settings_service_defaults_initialization() -> Result<(), AppError> {
        ...
        settings_service
            .set_int_setting("window.width", new_width)
            .await?;
