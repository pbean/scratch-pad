...
impl TestEnvironment {
    async fn new() -> Result<Self, AppError> {
        ...
        let app_state = AppState {
            ...
            global_shortcut: Arc::new(crate::global_shortcut::GlobalShortcutService::new_test(
                Arc::new(crate::settings::SettingsService::new(db_service.clone())),
            )?),
            window_manager: Arc::new(crate::window_manager::WindowManager::new_test(Arc::new(
                crate::settings::SettingsService::new(db_service.clone()),
            ))?),
...
mod note_command_tests {
    ...
    async fn test_get_note_service() -> Result<(), AppError> {
        ...
        // Create a note first
        let created_note = env
            .db_service
            .create_note("Test note for retrieval".to_string())
            .await?;
    ...
    async fn test_update_note_service() -> Result<(), AppError> {
        ...
        let created_note = env
            .db_service
            .create_note("Original content".to_string())
            .await?;

        // Test successful update
        let updated_note = env
            .db_service
            .update_note_content(created_note.id, "Updated content".to_string())
            .await?;
        ...
        let result = env
            .db_service
            .update_note_content(99999, "New content".to_string())
            .await;
    ...
    async fn test_delete_note_service() -> Result<(), AppError> {
        ...
        let created_note = env
            .db_service
            .create_note("Note to delete".to_string())
            .await?;
    ...
    async fn test_get_all_notes_service() -> Result<(), AppError> {
        ...
        let note2 = env
            .db_service
            .create_note("Second note".to_string())
            .await?;
    ...
    async fn test_get_notes_paginated_service() -> Result<(), AppError> {
        ...
        for i in 1..=10 {
            env.db_service
                .create_note(format!("Test note {}", i))
                .await?;
        }
    ...
    async fn test_note_content_validation() -> Result<(), AppError> {
        ...
        let valid_note = env
            .db_service
            .create_note("Valid note content".to_string())
            .await?;
    ...
    async fn test_note_metadata() -> Result<(), AppError> {
        ...
        let note = env
            .db_service
            .create_note("Test metadata".to_string())
            .await?;
        ...
        let updated_note = env
            .db_service
            .update_note_content(note.id, "Updated metadata".to_string())
            .await?;
    ...
    async fn test_concurrent_note_operations() -> Result<(), AppError> {
        ...
        let handles: Vec<_> = (1..=10)
            .map(|i| {
                ...
                tokio::spawn(
                    async move { service.create_note(format!("Concurrent note {}", i)).await },
                )
            })
            .collect();
...
mod search_service_tests {
    ...
    async fn test_search_service_basic_functionality() -> Result<(), AppError> {
        ...
        // Create test notes
        env.db_service
            .create_note("Rust programming tutorial".to_string())
            .await?;
        env.db_service
            .create_note("JavaScript async patterns".to_string())
            .await?;
        env.db_service
            .create_note("Python machine learning".to_string())
            .await?;
        ...
        assert!(results
            .iter()
            .any(|note| note.content.contains("programming")));

        // Test paginated search
        let (results, total_count) = env
            .app_state
            .search
            .search_notes_paginated("programming", 0, 10)
            .await?;
        assert!(!results.is_empty());
    ...
    async fn test_boolean_search_functionality() -> Result<(), AppError> {
        ...
        env.db_service
            ...
            .await?;
        env.db_service
            .create_note("Python programming basics".to_string())
            .await?;

        // Test Boolean search with AND
        let (results, _total_count, complexity) = env
            .app_state
            .search
...
mod settings_service_tests {
    ...
    async fn test_settings_service_basic_functionality() -> Result<(), AppError> {
        ...
        env.app_state
            .settings
            .set_bool_setting("bool_test", true)
            .await?;
