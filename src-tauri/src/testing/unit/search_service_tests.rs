...
mod search_service_integration_tests {
    ...
    async fn test_search_service_basic_search() -> Result<(), AppError> {
        ...
        // Create test notes
        let _note1 = search_service
            .db_service
            .create_note("Rust programming guide".to_string())
            .await?;
        let _note2 = search_service
            .db_service
            .create_note("JavaScript tutorial".to_string())
            .await?;
    ...
    async fn test_search_service_boolean_search() -> Result<(), AppError> {
        ...
        let _note2 = search_service
            .db_service
            .create_note("JavaScript programming guide".to_string())
            .await?;
        let _note3 = search_service
            .db_service
            .create_note("Database design patterns".to_string())
            .await?;
        let _note4 = search_service
            .db_service
            .create_note("Rust and JavaScript comparison".to_string())
            .await?;

        // Test Boolean AND search
        let (results, _total, complexity) = search_service
            .search_notes_boolean_paginated("Rust AND programming", 0, 10)
            .await?;
        assert_eq!(results.len(), 1);
        assert!(results[0].content.contains("Rust"));
        ...
        let (results, _total, _complexity) = search_service
            .search_notes_boolean_paginated("Database OR JavaScript", 0, 10)
            .await?;
        assert!(results.len() >= 2); // Should match Database, JavaScript, and possibly Rust+JavaScript notes

        // Test Boolean NOT search
        let (results, _total, _complexity) = search_service
            .search_notes_boolean_paginated("programming NOT JavaScript", 0, 10)
            .await?;
        assert_eq!(results.len(), 1);
        assert!(results[0].content.contains("Rust"));
    ...
    async fn test_search_service_basic_search_method() -> Result<(), AppError> {
        ...
        let _note1 = search_service
            .db_service
            .create_note("Rust programming".to_string())
            .await?;
        let _note2 = search_service
            .db_service
            .create_note("JavaScript development".to_string())
            .await?;
...
mod search_service_performance_tests {
    ...
    async fn test_search_service_empty_database() -> Result<(), AppError> {
        ...
        let (results, total) = search_service
            .search_notes_paginated("anything", 0, 10)
            .await?;
        assert_eq!(results.len(), 0);
        assert_eq!(total, 0);

        let (results, total, complexity) = search_service
            .search_notes_boolean_paginated("anything AND something", 0, 10)
            .await?;
        assert_eq!(results.len(), 0);
        assert_eq!(total, 0);
        assert!(complexity.is_valid);
    ...
    async fn test_search_service_special_characters() -> Result<(), AppError> {
        ...
        let _note1 = search_service
            .db_service
            .create_note("C++ programming language".to_string())
            .await?;
        let _note2 = search_service
            .db_service
            .create_note("Node.js runtime environment".to_string())
            .await?;
        let _note3 = search_service
            .db_service
            .create_note("SQL injection & security".to_string())
            .await?;
    ...
    async fn test_search_service_unicode_content() -> Result<(), AppError> {
        ...
        let _note1 = search_service
            .db_service
            .create_note("Rust プログラミング".to_string())
            .await?;
        let _note2 = search_service
            .db_service
            .create_note("JavaScript 🚀 development".to_string())
        ...
        let _note3 = search_service
            .db_service
            .create_note("Ñuméroüs spëcial châractërs".to_string())
            .await?;
