...
fn create_mock_db_service() -> std::sync::Arc<crate::database::DbService> {
    ...
    Arc::new(crate::database::DbService::new(&db_path).expect("Failed to create test database"))
}
...
mod tests {
    ...
    fn test_mock_service_factory_search_service() {
        let (search_service, note_repo, search_repo) =
            MockServiceFactory::create_mock_search_service();
