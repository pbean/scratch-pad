...
impl SearchRepository for MockSearchRepository {
    async fn execute_fts5_search(
        ...
        let mut matching_notes: Vec<Note> = all_data
            .values()
            .filter(|note| {
                note.content
                    .to_lowercase()
                    .contains(&fts5_query.to_lowercase())
            })
            .cloned()
            .collect();
...
mod tests {
    ...
    async fn test_mock_search_repository() {
        ...
        let advanced_results = repo
            .advanced_search(None, None, true, None, None, None)
            .await
            .unwrap();
    ...
    async fn test_error_responses() {
        ...
        repo.set_error_response(
            "search_favorites",
            AppError::General("Mock error".to_string()),
        );
