...
impl TestDatabase {
    ...
    async fn create_in_memory(test_id: u64) -> Result<Self, AppError> {
        ...
        let db_path = PathBuf::from(format!(
            "file:memdb_test_{}?mode=memory&cache=private",
            test_id
        ));
    ...
    async fn create_temp_file(test_id: u64) -> Result<Self, AppError> {
        let temp_file =
            NamedTempFile::with_prefix(&format!("test_db_{}_", test_id)).map_err(|e| {
    ...
    pub async fn populate_test_data(&self) -> Result<(), AppError> {
        ...
        db.set_setting(&format!("test_setting_{}", self.test_id), "test_value")
            .await?;
...
impl TestDatabaseFactory {
    ...
    pub async fn create_populated_db(
        strategy: IsolationStrategy,
    ) -> Result<TestDatabase, AppError> {
...
macro_rules! test_db {
    () => {{
        crate::testing::database::TestDatabaseFactory::create_memory_db()
            .await
            .unwrap()
    }};
    (file) => {{
        crate::testing::database::TestDatabaseFactory::create_file_db()
            .await
            .unwrap()
    }};
    (debug) => {{
        crate::testing::database::TestDatabaseFactory::create_debug_db()
            .await
            .unwrap()
    }};
    (populated) => {{
...
mod tests {
    ...
    async fn test_database_isolation() {
        ...
        db1.db()
            .create_note("Test note 1".to_string())
            .await
            .unwrap();
    ...
    async fn test_populated_database() {
        let test_db = TestDatabaseFactory::create_populated_db(IsolationStrategy::InMemory)
            .await
            .unwrap();
    ...
    async fn test_unique_timestamps() {
        ...
        assert_ne!(
            ts1, ts2,
            "Timestamps should be unique even in rapid succession"
        );
    }
